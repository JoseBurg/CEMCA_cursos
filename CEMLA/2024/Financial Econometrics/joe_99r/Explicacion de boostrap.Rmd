---
title: "Explicación del Código  estimación"
author: "José A. Burgos"
...
Este script en R replica la estimación, prueba e inferencia de efectos de umbral en paneles no dinámicos, como se reporta en el artículo "Threshold Effects in Non-Dynamic Panels: Estimation, Testing and Inference" de Bruce E. Hansen. A continuación, se desglosará cada línea del código y se ofrecerá una explicación teórica sobre su funcionamiento.
...
i <- invest[,1]   # inversión/activos                 
q <- invest[,2]   # Tobin's Q                      
c <- invest[,3]   # flujo de caja/activos                  
d <- invest[,4]   # deuda/activos
```

### Parámetros de configuración

Define los parámetros utilizados para el análisis, como número de cuantiles, nivel de confianza y configuraciones de bootstrap.

```{r}
qn <- 400          # número de cuantiles a examinar                
conf_lev <- 0.95   # nivel de confianza para el umbral                
vgraph <- 1        # establecer a 1 para graficar razones de verosimilitud           
boot_1 <- 300      # número de repeticiones, 0 para sin bootstrap, simple (300) 
boot_2 <- 300      # número de repeticiones, 0 para sin bootstrap, doble (300) 
boot_3 <- 300      # número de repeticiones, 0 para sin bootstrap, triple (300) 
trim_1 <- 0.01     # porcentaje a recortar antes de la búsqueda, simple      
trim_2 <- 0.01     # porcentaje a recortar antes de la búsqueda, doble      
trim_3 <- 0.05     # porcentaje a recortar antes de la búsqueda, triple      
max_lag <- 1
tt <- t-max_lag
ty <- n*(t-max_lag-1)
...
**lag_v**: Genera una matriz con los valores retrasados de una variable.
...
**tr**: Realiza la transformación de la serie temporal por resta de medias.
...
cf <- lag_v(c, 1) 
q1 <- lag_v(q, 1)
d1 <- lag_v(d, 1)   # definir como variable de umbral 
yt <- tr(y)
ct <- tr(cf)
...
+ `y, cf, q1, d1`: Preparación de variables retrasadas.

+ `x <- cbind(...)`: Combinación de variables explicativas.

+ `xt`: Aplicación de la transformación `tr` a cada columna de `x`.


### Función `sse_cal`

Calcula la suma de los errores cuadrados
...
Calcula SSE para un rango de valores de umbral.
